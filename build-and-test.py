...
Options:
  ...
  --build-all               Build All Module
  --build-test              Build Unit Test
  --build-assistant         Build UdiAssistant
  --build-tools             Build UdiUi and udicli
  --build-gui               Build UdiUi
  --build-cli               Build udicli
...
def read_version_from_properties(versionfile):
    with open(versionfile) as version:
        ...
        build_version = "0"
        return "U.{}.{}.{}.{}".format(properties["MAJOR_VERSION"],
...
def init_environ():
    if "UDI_SERVICE_VERSION" not in os.environ:
...
def install_conan_packages(arguments, build_path):
    ...
    if arguments["--build-all"]:
        install_command += " -o with_all=True"

    if arguments["--build-assistant"]:
        install_command += " -o with_assistant=True"

    if arguments["--build-tools"]:
        install_command += " -o with_tools=True"

    if arguments["--build-gui"]:
        install_command += " -o with_tool_gui=True"

    if arguments["--build-cli"]:
        install_command += " -o with_tool_cli=True"
...
def generate_build_script(arguments, build_path):
    ...
    source_path = os.getcwd()
    cmake_command = "cmake {} -B {} -S {}".format(cmake_generator, build_path,
                                                  source_path)
    ...
    if arguments["--build-all"]:
        cmake_command += " -DUDI_BUILD_ALL=ON"

    if arguments["--build-assistant"]:
        cmake_command += " -DUDI_BUILD_ASSISTANT=ON"

    if arguments["--build-tools"]:
        cmake_command += " -DUDI_BUILD_TOOLS=ON"

    if arguments["--build-gui"]:
        cmake_command += " -DUDI_BUILD_TOOL_GUI=ON"

    if arguments["--build-cli"]:
        cmake_command += " -DUDI_BUILD_TOOL_CLI=ON"

    if arguments["--build-test"]:
        cmake_command += " -DUDI_BUILD_TESTS=ON"

    if arguments["--check"]:
        cmake_command += " -DUDI_STATIC_CHECK=ON"
...
def print_html_coverage(index_html):
    with open(index_html, "r", encoding="utf-8") as ifile:
        for line in ifile:
            match = re.search(r"'labels', (\['Cover \d+%','Uncover \d+%'\])",
                              line)
            if not match:
                continue
...
def run_unit_test(arguments, build_path):
    ...
    if arguments["--coverage"]:
        ...
        if arguments["--coverage_ignore"]:
            excluded = "--excluded_sources {}".format(
                arguments["--coverage_ignore"])
        else:
            excluded = ""
    ...
    if arguments["--coverage"]:
        if arguments["--coverage_cobertura"]:
            ...
            print_html_coverage(os.path.join(os.getcwd(), outdir,
                                             "index.html"))
...
def main():
    ...
    if arguments["--gui"]:
        arguments["--build-tools"] = True
        arguments["--build-assistant"] = True

    if arguments["--build-tools"]:
        arguments["--build-gui"] = True
        arguments["--build-cli"] = True
